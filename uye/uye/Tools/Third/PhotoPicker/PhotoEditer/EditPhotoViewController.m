...
@property (nonatomic, strong) PhotoShapeView *shapeView;
@property (nonatomic, strong) UIButton *selectButton;
@property (nonatomic, strong) UIButton *cancelButton;
@property (nonatomic, copy) PhotoEditHandler editerHandler;
@end
...
- (void)editPhotoFinish:(PhotoEditHandler)complete {
    self.editerHandler = complete;
}

- (void)cancelDidClick {
    if (self.editerHandler) {
        self.editerHandler(nil,99);
    }
...
- (void)selectDidClick {
    if (self.editerHandler) {
        self.editerHandler(self.shapeImage,0);
    }
}
...
- (PhotoShapeView *)shapeView
{
    if(!_shapeView){
        _shapeView = [[PhotoShapeView alloc] init];
