...
@interface PhotoListManager ()
@property (nonatomic,strong)NSMutableArray <PhotoListModel *> *listArray;
@end
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.listArray = [NSMutableArray arrayWithArray:[self getPhotoAblumList]];
    }
    return self;
}
+ (void)requestPhotoAuthorizationStatus:(void (^)(BOOL))complete {
...
- (NSArray<PhotoListModel *> *)getPhotoAblumList
{
    
    NSMutableArray<PhotoListModel *> *photoAblumList = [NSMutableArray array];
    //获取所有智能相册
    PHFetchResult *cameraRollAlbums = [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeSmartAlbum subtype:PHAssetCollectionSubtypeSmartAlbumUserLibrary options:nil];
    [cameraRollAlbums enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull collection, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([collection.localizedTitle isEqualToString:@"All Photos"] ||[collection.localizedTitle isEqualToString:@"Camera Roll"] || [collection.localizedTitle isEqualToString:@"所有照片"]) {
            NSArray<PHAsset *> *assets = [self getAssetsInAssetCollection:collection ascending:NO];
            if (assets.count > 0) {
                PhotoListModel *ablum = [[PhotoListModel alloc] init];
    ...
    [smartAlbums enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull collection, NSUInteger idx, BOOL *stop) {
        ...
        if (!([collection.localizedTitle isEqualToString:@"Recently Deleted"] ||
              ...
              [collection.localizedTitle isEqualToString:@"Panoramas"]||
              [collection.localizedTitle isEqualToString:@"Slo-mo"]||
...
- (PhotoListModel *)cellModelWithIndexPath:(NSIndexPath *)indexPath {
    return [self.listArray objectAtIndex:indexPath.row];
}
